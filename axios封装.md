...
1、不同环境下的接口路径获取 process.env.NODE_ENV 
...
2、请求方法别名 一共7种 在使用别名方法时， `url`、`method`、`data` 这些属性都不必在配置中指定。
...
5、请求配置 这些是用于发出请求的可用配置选项。只有url是必需的。如果没有指定方法，请求将默认为get。
